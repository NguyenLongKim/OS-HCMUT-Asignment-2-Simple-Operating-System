...
struct pcb_t * get_proc(void) {
	 ...
	 * */
	pthread_mutex_lock(&queue_lock); // require lock
	if (empty(&ready_queue)){ //If ready queue is empty
		while (!empty(&run_queue)){ // push all processes in [run_queue] back to [ready_queue]
			enqueue(&ready_queue,dequeue(&run_queue));
		}
	}
	proc = dequeue(&ready_queue); // get pcb has highest priority
